...
- 久坐与低头提醒合并、独立冷却、会议模式、开始休息、延后、关闭和暂停；提示音内置三种可辨识选项，可按提醒级别自动匹配或手动指定。
- SQLite 设置、事件和每日汇总；7/30 日趋势、CSV 导出和不可逆的统计清空。
- 灵动岛提醒、托盘状态、提示音、单实例、开机启动、关闭到托盘和摄像头不可用后的定时模式。
- 天气与活动页：中国城市行政中心搜索、最多 8 个地点、当前天气与 5 日预报、15 分钟缓存及断网旧数据回退；首选地点同步显示在今日概览和灵动岛。
...
```text
React 19 + TypeScript strict + Tailwind CSS v4(CSS 变量设计令牌,仅交互与预览)
  ...
  └─ Tauri command / event(schemaVersion = 2)
       ...
       └─ Rust 领域状态机
            ├─ 消息表(messages.rs):所有面向用户的 zh/en 文案单一来源
            ├─ 提醒策略与提示音(sound.rs)、灵动岛提醒浮层
            ├─ SQLite 设置/事件/每日统计
            └─ 托盘、单实例与开机启动
```

摄像头帧只在 Rust 后端内存中短暂存在并压缩为 JPEG 预览帧，不会写入数据库、日志或文件。Rust 是行为判断的唯一事实来源，React 不自行决定是否提醒；提醒文案、错误提示和托盘菜单都由 Rust 侧消息表按界面语言生成，前端不做二次翻译。
...
`pnpm dev` 只在浏览器中运行 WebView 界面(便于样式与布局开发);桌面核心命令在浏览器中不可用,涉及摄像头或统计的交互会报错。
