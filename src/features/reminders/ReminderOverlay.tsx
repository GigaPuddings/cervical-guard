    ...
    <div className="pointer-events-none fixed inset-x-0 top-4 z-120 flex justify-center px-4" role="presentation">
      <section className="pointer-events-auto relative grid w-full max-w-117.5 grid-cols-[42px_minmax(0,1fr)_auto] items-center gap-3 overflow-hidden rounded-3xl border border-inverse/10 bg-panel-strong/98 px-3 py-2.5 text-inverse shadow-panel" role="dialog" aria-modal="false" aria-labelledby="reminder-title">
        ...
        <div className="relative flex items-center gap-1">
          ...
          <button className="grid size-8 place-items-center rounded-[10px] border border-inverse/10 bg-inverse/5 text-inverse-muted hover:bg-inverse/10" title="暂停一小时" aria-label="暂停一小时" onClick={onPause}>
            <Pause size={13} />
          </button>
          <button className="grid size-8 place-items-center rounded-[10px] border border-inverse/10 bg-inverse/5 text-inverse-muted hover:bg-inverse/10" title="忽略本次" aria-label="忽略本次" onClick={onDismiss}>
            <X size={13} />
          </button>
        </div>
      </section>
