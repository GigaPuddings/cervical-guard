...
export function HelpDialog({ open, language, onClose }: { open: boolean; language: Language; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[95] grid place-items-center bg-panel-strong/45 p-4 backdrop-blur-[3px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="flex max-h-[min(700px,calc(100vh-32px))] w-full max-w-[720px] flex-col overflow-hidden rounded-[24px] border border-edge bg-panel shadow-panel" role="dialog" aria-modal="true" aria-labelledby="help-dialog-title">
        <header className="flex items-start gap-4 border-b border-edge px-6 py-5">
          <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-accent-soft text-accent"><CircleHelp size={23} /></span>
          <div className="min-w-0 flex-1"><h2 className="m-0 text-lg font-black" id="help-dialog-title">{t.title}</h2><p className="mb-0 mt-1 text-[11px] text-muted">{t.subtitle}</p></div>
          <button className="grid size-9 place-items-center rounded-xl text-muted hover:bg-panel-muted hover:text-foreground" aria-label={t.close} onClick={onClose}><X size={18} /></button>
        </header>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {t.tips.map(([title, description, Icon]) => (
              <article className="flex gap-3 rounded-2xl border border-edge-soft bg-panel-muted p-4" key={title}>
                <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-panel text-accent shadow-control"><Icon size={17} /></span>
                <div><h3 className="m-0 text-xs font-extrabold">{title}</h3><p className="mb-0 mt-1.5 text-[9px] leading-[1.65] text-muted">{description}</p></div>
              </article>
            ))}
          </div>
          <p className="mb-0 mt-4 rounded-2xl bg-warning-soft px-4 py-3 text-[9px] leading-5 text-warning-foreground">{t.note}</p>
        </div>
        <footer className="flex justify-end border-t border-edge bg-panel-muted/55 px-6 py-4"><button className="min-h-10 rounded-xl bg-accent px-5 text-[11px] font-bold text-inverse hover:bg-accent-strong" onClick={onClose}>{t.close}</button></footer>
      </section>
    </div>
  );
}
