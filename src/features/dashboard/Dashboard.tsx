      ...
      <aside className={cn(
        ...
        <div className="mt-3 grid gap-0.5 px-2">
          <div className="mb-1 flex h-9 items-center justify-between gap-2 text-xs font-semibold text-muted">
            <span className="flex items-center gap-2"><Languages size={16} />{language === "en-US" ? "Language" : "界面语言"}</span>
            <span className="flex rounded-lg border border-edge bg-panel-muted p-0.5" role="group" aria-label={language === "en-US" ? "Interface language" : "界面语言"}>
              <button className={cn("rounded-md px-2 py-1 text-[10px] font-bold transition", language === "zh-CN" ? "bg-panel text-accent shadow-control" : "text-subtle hover:text-foreground")} aria-pressed={language === "zh-CN"} onClick={() => props.onLanguage("zh-CN")}>中</button>
              <button className={cn("rounded-md px-2 py-1 text-[10px] font-bold transition", language === "en-US" ? "bg-panel text-accent shadow-control" : "text-subtle hover:text-foreground")} aria-pressed={language === "en-US"} onClick={() => props.onLanguage("en-US")}>EN</button>
            </span>
          </div>
