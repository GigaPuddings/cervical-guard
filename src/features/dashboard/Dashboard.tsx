  ...
  return (
    <div className="relative mx-auto grid h-full min-h-0 w-full max-w-[1400px] grid-rows-[auto_minmax(0,1fr)] gap-3 overflow-hidden px-[clamp(16px,3vw,42px)] py-[clamp(12px,2.2vh,24px)]">
      ...
      <div className="grid min-h-0 grid-cols-[64px_minmax(0,1fr)] gap-3 min-[1080px]:grid-cols-[168px_minmax(0,1fr)]">
        ...
        {activeTab === "privacy" && <div className="grid gap-4 md:grid-cols-2"><div className="rounded-[14px] border border-edge-soft bg-panel-muted p-4"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-accent-soft text-accent"><ShieldCheck size={18} /></span><div><strong className="text-xs">原始画面不落盘</strong><p className="m-0 mt-0.5 text-[8px] text-muted">摄像头画面仅在本机内存中处理。</p></div></div><ul className="mt-3 grid list-none gap-2 p-0 text-[9px] text-muted [&_li]:flex [&_li]:items-center [&_li]:gap-2 [&_svg]:text-accent"><li><Eye size={14} /> 不进行身份识别</li><li><CameraOff size={14} /> 不保存视频或截图</li><li><LockKeyhole size={14} /> 不上传摄像头数据</li></ul><Toggle checked={draft.statisticsEnabled} onChange={(value) => set("statisticsEnabled", value)} label="保存本地行为统计" description="关闭后停止累计，已有数据不自动删除" /></div><div className="rounded-[14px] border border-edge-soft bg-panel-muted p-4"><strong className="text-xs">本地数据管理</strong><p className="mb-3 mt-1 text-[9px] leading-4 text-muted">导出内容仅包含日期、时长、次数与结构化行为历史，不含图片。</p><div className="grid gap-2"><button className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-edge bg-panel px-4 text-[10px] font-bold text-muted hover:bg-panel-muted" onClick={onExport}><Download size={16} /> 导出 CSV</button><button className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-4 text-[10px] font-bold text-danger hover:bg-danger-soft" onClick={onDeleteData}><Trash2 size={16} /> 删除全部统计与行为历史</button></div><p className="mb-0 mt-3 text-[8px] leading-3.5 text-subtle">健康提醒用于日常行为提醒，不用于疾病诊断或替代医生建议。</p></div></div>}
        </section>
      </div>
