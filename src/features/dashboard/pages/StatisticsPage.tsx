...
const statisticsMessages = defineMessages({
  ...
  reminderAction: '提醒操作',
  averageSitting: '日均坐姿',
  stableSitting: '稳定坐姿累计',
  longest: '最长连续',
  reduceGradually: '逐步缩短即可',
...
export function StatisticsPage({ statistics, history, snapshot, onHistoryDate }: { statistics: DailyStatistics[]; history: BehaviorHistoryEvent[]; snapshot: AppSnapshot; onHistoryDate: (localDate: string) => Promise<void> }) {
  ...
  const metrics = [
    { label: messages.averageSitting, value: compactDuration(totalSeated / Math.max(1, rows.length), language), note: messages.stableSitting, icon: Armchair },
    { label: messages.longest, value: compactDuration(Math.max(0, ...rows.map(item => item.longestSeatedSeconds)), language), note: messages.reduceGradually, icon: Clock3 },
    { label: messages.cumulativeHeadDown, value: compactDuration(totalHeadDown, language), note: `${totalReminders} ${messages.reminders}`, icon: TimerReset },
    { label: messages.awayActivity, value: compactDuration(totalAway, language), note: `${rows.reduce((sum, item) => sum + item.awayCount, 0)} ${messages.awayEvents}`, icon: PersonStanding },
    { label: messages.completedBreaks, value: `${totalBreaks} ${messages.times}`, note: `${Math.min(100, completion)}% ${messages.conversion}`, icon: Coffee },
    { label: messages.ignoredReminders, value: `${totalDismissed} ${messages.times}`, note: totalReminders ? `${Math.round((totalDismissed / totalReminders) * 100)}% ${messages.ofReminders}` : messages.noReminders, icon: Bell }
  ]
  ...
  return (
    <div className="mx-auto grid h-full min-h-0 w-full max-w-[1680px] grid-rows-[auto_auto_minmax(0,1fr)] gap-5 overflow-hidden px-[clamp(24px,3.2vw,52px)] py-[clamp(22px,3vh,38px)]">
      <header className="flex shrink-0 items-end justify-between gap-5">
        <div>
          <span className="text-xs font-extrabold tracking-[.14em] text-accent">{messages.localOnly}</span>
          <h1 className="mb-0 mt-1.5 text-[clamp(28px,2.2vw,36px)] font-black leading-none tracking-[-.04em]">{messages.title}</h1>
          <p className="mb-0 mt-2.5 text-[13px] text-muted">{messages.subtitle}</p>
        </div>
        <div className="flex rounded-xl bg-edge-soft p-0.5 text-[11px] font-bold">
          <button className={cn('min-h-9 rounded-[10px] px-4 text-muted transition hover:text-foreground', range === 7 && 'border border-accent/55 bg-panel text-accent shadow-control')} aria-pressed={range === 7} onClick={() => setRange(7)}>{messages.last7Days}</button>
          <button className={cn('min-h-9 rounded-[10px] px-4 text-muted transition hover:text-foreground', range === 30 && 'border border-accent/55 bg-panel text-accent shadow-control')} aria-pressed={range === 30} onClick={() => setRange(30)}>{messages.last30Days}</button>
        </div>
      </header>
      ...
      <div className="grid min-h-0 gap-4 min-[1080px]:grid-cols-[minmax(0,1.7fr)_minmax(310px,.78fr)]">
        <section className="flex min-h-0 flex-col rounded-2xl border border-edge bg-panel px-5 pb-4 pt-2.5 shadow-panel 2xl:px-6">
          ...
          <div className="mt-3 flex min-h-53 flex-1 flex-col">
            <div className="flex items-center justify-between text-[10px] text-muted"><span>{messages.sittingMinutes}</span><span>{messages.headDownMinutes}</span></div>
                  ...
                  return (
                    <div className="group relative flex h-full min-w-0 flex-1 flex-col items-center justify-end" key={item.localDate}>
                      <div className="pointer-events-none absolute left-1/2 top-1 z-20 w-max max-w-44 -translate-x-1/2 rounded-lg bg-panel-strong px-3 py-2 text-[9px] leading-4 text-inverse opacity-0 shadow-panel transition group-hover:opacity-100">
                        ...
                        <span className="block">{messages.sitting} {compactDuration(item.seatedSeconds, language)}</span>
                        <span className="block">{messages.headDownShort} {compactDuration(item.headDownSeconds, language)}</span>
                      </div>
          ...
          <div className="mt-4 flex shrink-0 items-center gap-3 rounded-xl border border-edge-soft bg-panel-muted px-4 py-3">
            ...
            <p className="min-w-0 truncate text-[10px] text-muted"><b className="mr-3 text-foreground">{messages.observation}</b>{snapshot.today.breakCount > 0 ? messages.positiveObservation : messages.emptyObservation}</p>
          </div>
        </section>
        ...
        <section className="flex min-h-0 flex-col overflow-visible rounded-2xl border border-edge bg-panel px-5 pb-4 pt-2.5 shadow-panel">
          <div className="relative z-20 flex h-12 shrink-0 items-center justify-between border-b border-edge-soft" ref={calendarRef}>
            ...
            {calendarOpen && (
              <div className="absolute right-0 top-[52px] z-30 w-[min(320px,calc(100vw-32px))] overflow-hidden rounded-[20px] border border-edge bg-panel shadow-[0_22px_60px_rgba(25,48,33,.2)]" role="dialog" aria-label={messages.historyDate}>
                <div className="grid grid-cols-[32px_32px_minmax(0,1fr)_32px_32px] items-center border-b border-edge-soft px-3 py-3">
                  <button className="grid size-8 place-items-center rounded-lg text-subtle transition hover:bg-panel-muted hover:text-foreground" aria-label={messages.previousYear} onClick={() => moveCalendar(-12)}><ChevronsLeft size={17} /></button>
                  <button className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-panel-muted hover:text-foreground" aria-label={messages.previousMonth} onClick={() => moveCalendar(-1)}><ChevronLeft size={17} /></button>
                  <strong className="text-center text-[13px] font-black tracking-[.04em]">{calendarMonthLabel}</strong>
                  <button className="grid size-8 place-items-center rounded-lg text-muted transition hover:bg-panel-muted hover:text-foreground" aria-label={messages.nextMonth} onClick={() => moveCalendar(1)}><ChevronRight size={17} /></button>
                  <button className="grid size-8 place-items-center rounded-lg text-subtle transition hover:bg-panel-muted hover:text-foreground" aria-label={messages.nextYear} onClick={() => moveCalendar(12)}><ChevronsRight size={17} /></button>
                </div>
                ...
                <button className="flex min-h-11 w-full items-center justify-center border-t border-edge-soft bg-panel-muted text-[11px] font-bold text-accent transition hover:bg-accent-soft" onClick={() => selectCalendarDate(today)}>{messages.todayButton}</button>
              </div>
            )}
          </div>
          ...
          >
            {historyLoading ? (
              <div className="grid h-full min-h-40 place-content-center justify-items-center gap-3 text-center text-muted" role="status"><LoaderCircle className="animate-spin text-accent" size={22} /><span className="text-[11px]">{messages.loadingHistory}</span></div>
            ) : dayHistory.length ? (
              <div
                className="relative"
                role="list"
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <small className="mt-1 flex items-center gap-1.5 truncate text-[9px] text-muted"><i className={cn('size-1.5 shrink-0 rounded-full', copy.dot)} />{Number.isNaN(when.getTime()) ? event.startedAt : when.toLocaleTimeString(language, { hour: '2-digit', minute: '2-digit' })}{event.action ? ` · ${historyActionLabel(event.action, actionMessages)}` : ''}</small>
                      </div>
              ...
              <div className="grid place-content-center justify-items-center gap-3 text-center text-muted"><span className="grid size-12 place-items-center rounded-2xl bg-panel-muted"><CalendarDays size={24} /></span><span className="text-[11px]">{selectedDate === today ? messages.noHistoryToday : `${selectedDateLabel} ${messages.noHistory}`}</span></div>
            )}
          </div>
        </section>
      </div>
