...
export function WeatherDetail({ location, forecast, error, loading, onRefresh, onRemove }: {
...
function WeatherDetailContent({ forecast }: { forecast: WeatherForecast }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2.5 p-3">
      ...
      <div className="grid min-h-[58px] flex-1 grid-cols-[minmax(0,1.2fr)_minmax(145px,.8fr)] gap-2.5"><div className="flex min-w-0 items-center gap-3 rounded-xl border border-info/15 bg-info-soft/65 px-3 py-2 text-info"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-panel"><Activity size={17} /></span><div className="min-w-0"><strong className="block text-[clamp(10px,.72vw,13px)]">结合天气的休息建议</strong><p className="mt-1 text-[clamp(10px,.72vw,13px)] leading-[1.55]">{weatherHealthAdvice(forecast)}</p></div></div><div className="flex min-w-0 items-center gap-3 rounded-xl border border-edge bg-panel-muted px-3 py-2"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-panel text-accent"><SunMedium size={17} /></span><div className="min-w-0"><span className="block text-[clamp(9px,.65vw,11px)] text-muted">今日环境范围</span><strong className="mt-1 block text-[clamp(13px,1vw,17px)]">{Math.round(today?.temperatureMin ?? 0)}–{Math.round(today?.temperatureMax ?? 0)}°</strong><small className="mt-0.5 block truncate text-[clamp(8px,.6vw,11px)] text-muted">降水 {today?.precipitationProbability.toFixed(0) ?? 0}% · UV 峰值 {today?.uvIndexMax.toFixed(1) ?? "0.0"}</small></div></div></div>
      <div className="grid h-[clamp(82px,13vh,104px)] shrink-0 grid-cols-5 overflow-hidden rounded-xl border border-edge bg-panel-muted/70">{forecast.daily.map((day, index) => <div key={day.date} className="grid min-w-0 place-content-center justify-items-center gap-1 border-r border-edge px-1 text-center last:border-r-0"><span className="text-[clamp(9px,.65vw,11px)] font-bold text-muted">{index === 0 ? "今天" : new Intl.DateTimeFormat("zh-CN", { weekday: "short" }).format(new Date(`${day.date}T12:00:00`))}</span><span className="text-info"><WeatherGlyph code={day.weatherCode} size={17} /></span><strong className="text-[clamp(10px,.78vw,13px)]">{Math.round(day.temperatureMax)}° <span className="font-normal text-muted">{Math.round(day.temperatureMin)}°</span></strong><small className="truncate text-[clamp(8px,.58vw,10px)] text-info">雨 {day.precipitationProbability.toFixed(0)}% · UV {day.uvIndexMax.toFixed(1)}</small></div>)}</div>
