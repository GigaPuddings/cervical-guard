...
export function formatWeatherUpdatedAt(forecast: WeatherForecast): string {
  ...
  return Number.isNaN(date.getTime())
    ? "更新时间未知"
    : `${new Intl.DateTimeFormat("zh-CN", { hour: "2-digit", minute: "2-digit" }).format(date)} 更新`;
}
...
export function weatherHealthAdvice(forecast: WeatherForecast, context: "overview" | "break" = "overview"): string {
  ...
  if (forecast.current.precipitation > 0 || rainProbability >= 70) return context === "break"
    ? "有降水，休息时可在室内走动，避免久坐后立即冒雨外出。"
    : "有降水，起身活动优先选择室内通道。";
  if (uvIndex >= 8) return context === "break"
    ? "紫外线很强，这轮休息在室内放松肩颈更稳妥。"
    : "紫外线很强，短时外出也要做好遮阳。";
