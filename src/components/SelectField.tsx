...
export function SelectField<T extends string | number>({ value, options, onChange, ariaLabel, disabled = false, placement = 'bottom', className }: SelectFieldProps<T>) {
    ...
    <div ref={rootRef} className={cn('relative min-w-0', className)}>
      ...
      {open && (
        <div
          role="listbox"
          aria-label={ariaLabel}
          className={cn('absolute left-0 right-0 z-260 max-h-60 overflow-y-auto rounded-xl border border-edge bg-panel p-1.5 shadow-[0_18px_46px_rgba(25,48,31,.16)]', placement === 'top' ? 'bottom-[calc(100%+7px)]' : 'top-[calc(100%+7px)]')}
          onKeyDown={event => {
            if (event.key === 'Escape') {
              event.preventDefault()
              event.stopPropagation()
          ...
          {options.map((option, index) => {
            ...
            return (
              <button
                ...
                className={cn('flex min-h-10 w-full items-center justify-between gap-3 rounded-lg px-3 text-left text-[13px] text-muted outline-none transition 2xl:min-h-11 2xl:text-sm', 'hover:bg-panel-muted hover:text-foreground focus-visible:bg-accent-soft focus-visible:text-accent-strong', isSelected && 'bg-accent-soft font-bold text-accent-strong')}
