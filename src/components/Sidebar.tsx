...
export function Sidebar({
  ...
  return (
    <aside className={cn('app-sidebar fixed inset-y-0 left-0 z-40 flex w-[240px] flex-col border-r border-edge bg-sidebar px-5 pb-5 pt-6 transition-transform md:static md:w-full md:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
      <div className="flex items-center justify-between px-2">
        <Brand language={language} />
        <button className="grid size-8 place-items-center rounded-[10px] text-muted hover:bg-panel-muted md:hidden" aria-label={messages.closeNavigation} onClick={onClose}><X size={17} /></button>
      </div>
      ...
      <div className="mt-auto">
        <button className="flex w-full items-center gap-3 rounded-[14px] border border-edge bg-panel px-3 py-3 text-left transition hover:border-accent/25 hover:bg-panel-muted" onClick={() => onPage('privacy')}>
          <span className="grid size-9 shrink-0 place-items-center rounded-[12px] bg-accent-soft text-accent"><LockKeyhole size={17} /></span>
        ...
        <div className="mt-3 grid gap-0.5 px-2">
          <div className="flex h-9 items-center justify-between gap-2 text-[11px] font-semibold text-muted">
            <span className="flex items-center gap-2"><Languages size={15} />{messages.interfaceLanguage}</span>
            <span className="flex rounded-[8px] border border-edge bg-panel-muted p-0.5" role="group" aria-label={messages.interfaceLanguage}>
              <button className={cn('rounded-[6px] px-2 py-1 text-[9px] font-bold', language === 'zh-CN' ? 'bg-panel text-accent shadow-control' : 'text-subtle')} aria-pressed={language === 'zh-CN'} onClick={() => onLanguage('zh-CN')}>{copy['zh-CN'].settings.chineseShort}</button>
              <button className={cn('rounded-[6px] px-2 py-1 text-[9px] font-bold', language === 'en-US' ? 'bg-panel text-accent shadow-control' : 'text-subtle')} aria-pressed={language === 'en-US'} onClick={() => onLanguage('en-US')}>EN</button>
            </span>
          </div>
          ...
          <button className="flex h-8 items-center gap-2 text-[11px] font-semibold text-muted hover:text-accent" onClick={onHelp}><CircleHelp size={15} /> {messages.help}</button>
          <p className="mt-1 text-[9px] text-subtle">{copy[language].appName} v{packageJson.version} · {messages.behaviorTool}</p>
        </div>
      </div>
    </aside>
  )
