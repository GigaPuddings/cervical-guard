...
class MockCore {
  ...
  private reminderCadenceSeconds(): number {
    return this.snapshot.settings.sedentarySeconds <= 30
      ? this.snapshot.settings.sedentarySeconds
      : this.snapshot.settings.repeatReminderMinutes * 60;
  }
  ...
  private updateReminderSchedule(now: number): void {
    ...
    if (this.snoozedUntil && this.snoozedUntil > now) {
      ...
      remaining = Math.max(0, Math.ceil(
        this.reminderCadenceSeconds() - (now - this.lastSedentaryReminderAt) / 1_000,
      ));
    } else {
      remaining = null;
    }
    this.snapshot.reminderRemainingSeconds = remaining;
    const clockRunning = this.snapshot.monitoringMode === "timer"
      || (this.snapshot.personPresent
        && (this.snapshot.behavior === "sitting_normal" || this.snapshot.behavior === "head_down"));
    this.snapshot.nextReminderAt = remaining === null || !clockRunning
      ? null
      : new Date(Date.now() + remaining * 1_000).toISOString();
  }
  ...
  private tick(): void {
    ...
    if (this.snapshot.lifecycle === "monitoring" || this.snapshot.lifecycle === "degraded") {
      ...
      if (personHere && (timerMode || this.snapshot.behavior === "sitting_normal" || this.snapshot.behavior === "head_down")) {
        ...
        this.snapshot.today.longestSeatedSeconds = Math.max(
          this.snapshot.today.longestSeatedSeconds,
          this.snapshot.seatedSeconds,
        );
        const repeatDue = this.lastSedentaryReminderAt !== null
          && this.snapshot.settings.repeatReminders
          && (now - this.lastSedentaryReminderAt) / 1_000 >= this.reminderCadenceSeconds();
        if (!this.snapshot.currentReminder
          && this.snapshot.seatedSeconds >= this.snapshot.settings.sedentarySeconds
          && (this.lastSedentaryReminderAt === null || repeatDue)) {
  ...
  private csv(): string {
    ...
    return [
      "日期,坐姿秒数,最长连续坐姿秒数,低头秒数,休息次数,提醒次数,忽略次数,离座秒数",
      ...rows.map((item) =>
        [
          item.localDate,
          Math.round(item.seatedSeconds),
          Math.round(item.longestSeatedSeconds),
          Math.round(item.headDownSeconds),
          item.breakCount,
          item.reminderCount,
          item.dismissedCount,
          Math.round(item.awaySeconds),
        ].join(","),
      ),
    ].join("\n");
  }
