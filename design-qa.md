...
**Forest-background and decorative-message correction — 2026-09-20**

- Source visual truth: `C:\Users\zero\AppData\Local\Temp\codex-clipboard-e9ebf142-9b45-4b7c-a1a4-71c999480b33.png` (`1670 x 942`). The follow-up annotation is `C:\Users\zero\AppData\Local\Temp\codex-clipboard-50283fc4-454b-4ff1-bfcc-f3e7e4dd637c.png` (`2012 x 1134`) and marks the window chrome, header decoration, and first two metric accents. Image 4 remains the independent forest background and Image 3 remains the independent transparent decorative-message asset.
- Rendered implementation: `output/design-qa/today-forest-chrome-after-1341x756.png`, captured from the real Tauri desktop window at its `1341 x 756` logical default size. The target's `1670 x 942` physical image was bicubically normalized to the same `1341 x 756` comparison size; the source and implementation aspect ratios differ by less than `0.1%`.
- Same-input full-view evidence: `output/design-qa/today-forest-chrome-comparison.png` places the normalized target on the left and the implementation on the right. Both show the paused state. Temperature, weather condition, humidity, and wind remain live and intentionally differ.
- Focused evidence: `output/design-qa/today-forest-chrome-focused.png` compares the titlebar/application mark, the independently positioned leaf/message area, and the first two metric cards in one input.
- Typography: the Microsoft YaHei UI/PingFang SC stack, title hierarchy, accent treatment on `姿势`, posture title, timer, labels, and metric values match the target hierarchy without broken wrapping or truncation.
- Spacing and layout: the titlebar is now `40px` high, matching the density-normalized target and shifting the dashboard to the correct vertical origin. Sidebar width, header regions, primary-card proportions, camera column, compact advice row, and five-card metric track remain aligned with no page-level overlap or horizontal clipping.
- Colors and tokens: the page uses a dark forest canvas, translucent green panels, restrained borders, soft shadows, muted secondary text, and health-green accents. Cards remain readable while allowing the supplied background atmosphere to show through.
- Image quality and asset fidelity: the first follow-up removed the earlier right-expanded `cover` crop. The final horizontal leaf calibration is documented below. No leaves appear in the top-left or bottom-right. Image 3 remains a separate transparent `<img>` layer between the title and weather regions; it is not baked into the background, recolored, redrawn, or given glow/border effects. The titlebar uses the real `favicon.svg` asset instead of the earlier generic green dot.
- Copy and content: static Chinese labels match the source. Current date, temperature, humidity, wind, and runtime statistics are product data and remain dynamic.
- Icons and controls: existing Lucide controls remain consistent. An intermediate visual pass restored short green/amber metric accents from the static mock; the later semantic correction below supersedes that treatment for zero values. The advice chevron and selected navigation treatment remain unchanged.
- Accessibility and behavior: the decorative message has empty alternative text plus `aria-hidden`; it cannot intercept pointers. Existing semantic buttons, focus handling, navigation, pause/resume, rest, privacy, and settings actions remain intact.
- Comparison history: pass 1 found the initial background, spacing, chevron, and metric hierarchy drift. The user's follow-up annotation then exposed three remaining P2 issues: generic/flat titlebar treatment, leaf bleed into the primary card from an over-shifted crop, and incorrect zero-state metric accents. That pass restored the favicon/background continuity and tone-aware green/amber accents; the later surface-color calibration below supersedes its temporary primary-card opacity treatment.
- Verification: `pnpm test -- --reporter=verbose` passed 19 files / 69 tests; `pnpm build` passed; `git diff --check` reported no whitespace errors (only existing LF-to-CRLF notices). The existing Vite chunk-size advisory remains non-blocking and unrelated to this visual correction.
- Remaining P3: the target camera-preview artwork contains faint internal plant silhouettes. They were intentionally not copied from the global forest background because the explicit constraint prohibits unintended bottom-right leaves, and the supplied Image 4 must not be reused as a generic decorative texture.

**Today surface-color and decoration calibration — 2026-09-20**

- Source visual truth: `C:\Users\zero\AppData\Local\Temp\codex-clipboard-e9ebf142-9b45-4b7c-a1a4-71c999480b33.png` (`1670 x 942`). The user's annotated side-by-side evidence is `C:\Users\zero\AppData\Local\Temp\codex-clipboard-e4ba1553-3179-49f6-9b43-ced01f879093.png` (`1840 x 528`), with the lighter target on the left and the darker implementation on the right.
- Rendered implementation: `output/design-qa/today-surface-decoration-after-1341x756.png`, captured from the real Tauri desktop window at `1341 x 756` logical pixels. The target was bicubically normalized from `1670 x 942` to `1341 x 756` for comparison.
- Full-view evidence: `output/design-qa/today-surface-decoration-comparison.png` places the normalized target on the left and final implementation on the right. Focused evidence: `output/design-qa/today-surface-decoration-focused.png` compares the leaf/art region and main-card surface in one input.
- Colors and tokens: the earlier primary-card-specific `rgba(12, 29, 20, 0.93)` override was removed. The Today page now uses a scoped translucent forest-gray panel token, with a slightly quieter scoped sidebar panel token. Measured median RGB is `23/39/36` target versus `22/39/35` implementation for the main card, and `22/37/33` target versus `22/37/34` implementation for the metric row.
- Image quality and positioning: the forest asset remains independent from the decorative-message asset. Two layers of the same supplied forest bitmap keep the image at its original apparent scale: the front layer is shifted left `2.5vw` to align the leaf cluster, while the rear right-anchored layer only fills the exposed right edge so the source glow is not replaced with CSS art. No leaf is introduced at the top-left or bottom-right.
- Decorative typography asset: Image 3 remains a standalone, pointer-transparent `<img>`. Its final measured visible box is `120 x 66px` at `x=760`, compared with the target `121 x 69px` at `x=760`; the remaining three-pixel height difference is antialiasing/transparency-edge noise and is not visually actionable.
- Typography, spacing, and copy: card geometry, heading sizes, navigation, live-state copy, weather placement, metric values, and all interaction tracks were preserved. Only surface tokens and the two independent decorative image placements changed.
- Comparison history: the annotated comparison identified P2 surface darkness, horizontal leaf drift, and oversized/high decorative lettering. Pass 1 removed the excessive primary-card opacity and brought card medians to within three RGB values. Pass 2 restored source-scale leaves with a left-shifted/fill-layer composition and calibrated the art asset. A tested `-40px` vertical background compensation visibly over-raised the leaves, so it was rejected and reverted. The final full and focused comparisons contain no actionable P0, P1, or P2 differences in the annotated regions.
- Verification: `pnpm test -- --reporter=verbose` passed 19 files / 69 tests; `pnpm build` passed; `git diff --check` reported no whitespace errors (only existing LF-to-CRLF notices). The existing Vite chunk-size advisory remains non-blocking and unrelated.

**Metric progress semantics and primary CTA color correction — 2026-09-20**

- Updated annotation: `C:\Users\zero\AppData\Local\Temp\codex-clipboard-dddea074-4394-455c-90bb-646837bd2f45.png` (`2012 x 1134`) marks the two zero-value metric accents in red and the Resume Detection / Start Break buttons in green.
- Rendered implementation: `output/design-qa/today-progress-button-after-1341x756.png`, captured from the real Tauri desktop window at `1341 x 756`. Focused evidence: `output/design-qa/today-progress-button-focused.png`; target/before is on the left and final implementation is on the right.
- Confirmed semantic defect: both metric values were `0`, but the component forced `42px` and `22px` colored minimum fills. This visually implied non-zero progress. The second metric also compared cumulative head-down time against the unrelated sedentary reminder threshold solely to produce that decoration.
- Progress fix: zero and negative progress now render no filled segment; positive progress is clamped to `0–100` and rendered at its exact percentage without a decorative minimum. Cumulative head-down no longer exposes a progress bar until a product-defined target exists.
- CTA color fix: the two highlighted controls now use a dedicated primary-action surface rather than the muted global accent. Measured Resume Detection color is `129/241/180` target versus `129/240/181` final; Start Break is `127/236/178` target versus `128/240/181` final. Both buttons now use the target-aligned dark ink foreground.
- Layout, copy, icons, and behavior: button geometry, Lucide icons, click handlers, card sizes, notes, and all other controls remain unchanged. The fix does not alter stored statistics or reminder logic.
- Comparison history: the annotation identified one P2 semantic contradiction and one P2 visual-token mismatch. The false minimum widths and unrelated second-card denominator were removed; dedicated CTA colors were sampled from the source. The final focused comparison shows correct zero states and matching button color, with no actionable P0, P1, or P2 issue remaining in the marked regions.
- Verification: the new `MetricCard` regression tests confirm that `0%` emits no progress element and `25%` emits exactly `width:25%`. `pnpm test -- src/components/MetricCard.test.tsx --reporter=verbose` passed 20 files / 71 tests; `pnpm build` passed. The existing Vite chunk-size advisory remains non-blocking and unrelated.

**Dynamic Island session-timer synchronization — 2026-09-20**

- Source visual truth: `C:\Users\zero\AppData\Local\Temp\codex-clipboard-c4b476b5-3539-4ca5-8f75-093fe9b7de99.png` (`2012 x 1134`) establishes the paused main-page current-session value and compact `0:00` clock format. Defect evidence is `C:\Users\zero\AppData\Local\Temp\codex-clipboard-671fa047-a8df-4ca0-b134-cfaac0285cbc.png` (`540 x 264`), where the independent island detail view incorrectly displays `272 小时 10 分钟` over two oversized lines.
- Rendered implementation: `output/design-qa/island-detail-timer-fixed.png`, captured from the actual `public/island.html` template in the paused detail state at its native `360 x 176` CSS window size and device density `1`. The real Tauri binary was rebuilt before capture because the island template is embedded with `include_bytes!`.
- Same-input focused evidence: `output/design-qa/island-timer-sync-comparison.png` places the main-page timer baseline and final island timer together. The full island-detail implementation is also preserved in `output/design-qa/island-detail-full-viewport.png`.
- Confirmed P1 defect: the paused island used `Date.now() - (pausedStartedAt || lastDetectionAt)` while the main page used `snapshot.seatedSeconds`. After restart, the fallback `lastDetectionAt` could be days old, producing the reported hundreds-of-hours value even though the current session showed `0:00`.
- Data and format correction: the island now reads `snapshot.seatedSeconds`, exactly like the main current-session ring, and formats it as total minutes plus zero-padded seconds. The left label remains `连续坐姿`; the separate right column retains the last pause clock time.
- Typography and layout: the island timer is reduced from `25px` to `22px`, uses tabular numerals, and is forced to one line. At `0:00` it fits the left detail column without wrapping, clipping, or displacing the three statistic cards.
- Colors, imagery, icons, and copy: existing forest-black surface, green status dot, weather strip, borders, and supporting-stat styling remain unchanged. No image or icon substitution was introduced. The only copy correction is removal of the misleading `距暂停离座` label from the current-session value.
- Comparison history: the first evidence showed a P1 value-source mismatch and P2 wrapping/hierarchy problem. The final capture shows `连续坐姿 0:00`, matches the main-page session value, keeps `上一次暂停时间` independent, and has no actionable P0, P1, or P2 issue in the timer region.
- Verification: `node --test scripts/island-template.node-test.mjs` passed 2 tests; `pnpm test` passed 20 files / 71 tests; `pnpm build` passed. The existing Vite chunk-size advisory remains non-blocking and unrelated.
